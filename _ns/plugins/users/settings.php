...
if (isset($_POST) && isset($_POST['ed_button']) && $_POST['ed_button']) {
	...
	if ($_POST['email'] && ($_POST['email'] != $old_values['email'])) {
		...
		if ($err = validate_input(['check' => 'unique', 'input' => 'email', 'field' => 'email', 'table' => 'ns_users', 'error' => __('This e-mail address is already registered.'), 'this_id' => $user_info['id']])) {
			$error_array['email'] = $err;
			$errors = true;
		}

		if ($err = validate_input(['check' => 'email', 'input' => 'email', 'error' => __('This is not a valid e-mail address.')])) {
			$error_array['email'] = $err;
			$errors = true;
		}
		$fields[] = 'email';
	...
	if ($_POST['oldpass'] || $_POST['newpass'] || $_POST['newpass2']) {
		...
		if (hash('sha512', $_POST['oldpass'].$old_values['salt']) != $old_values['password']) {
			$error_array['oldpass'] = __('That is not the correct password.');
			$errors = true;
		}

		if ($err = validate_input(['check' => 'empty', 'input' => 'newpass', 'error' => __('When changing your password, the new password can\'t be blank.')])) {
		  $error_array['newpass'] = $err;
		  $errors = true;
		}

		if ($err = validate_input(['check' => 'matching', 'input2' => 'newpass', 'input' => 'newpass2', 'error' => __('The "Password" and "Confirm password" fields don\'t match.')])) {
