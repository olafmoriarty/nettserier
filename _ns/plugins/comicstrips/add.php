<?php
	...
	if (isset($_POST) && isset($_POST['upload']) && $_POST['upload']) {
		...
		for ($i = 0; $i < $imgnum; $i++) {
			...
			if ($_FILES['file']['tmp_name'][$i]) {
				...
				if (getimagesize($_FILES['file']['tmp_name'][$i]) === false) {
					...
					if (isset($error_array['file[]'])) {
						$error_array['file[]'] .= '<br>'."\n".str_replace('{file}', htmlspecialchars($_FILES['file']['name'][$i]), __('{file} is not a valid image file.'));
					}
					else {
						$error_array['file[]'] = __('Could not upload file(s):').'<br>'."\n".str_replace('{file}', htmlspecialchars($_FILES['file']['name'][$i]), __('{file} is not a valid image file.'));
					}
				}
				...
				if ($_FILES['file']['size'][$i] > $max_file_size) {
					$errors = true;
					if (isset($error_array['file[]'])) {
						$error_array['file[]'] .= '<br>'."\n".str_replace(['{file}', '{size}'], [htmlspecialchars($_FILES['file']['name'][$i]), floor($max_file_size / 1024)], __('{file} exceeds the maximum allowed filesize of {size} kB.'));
					}
					else {
						$error_array['file[]'] = __('Could not upload file(s):').'<br>'."\n".str_replace(['{file}', '{size}'], [htmlspecialchars($_FILES['file']['name'][$i]), floor($max_file_size / 1024)], __('{file} exceeds the maximum allowed filesize of {size} kB.'));
					}
				...
				if (!in_array($extension, $accepted_extensions)) {
					$errors = true;
					if (isset($error_array['file[]'])) {
						$error_array['file[]'] .= '<br>'."\n".str_replace(['{file}', '{extensions}'], [htmlspecialchars($_FILES['file']['name'][$i]), implode(', ', $accepted_extensions)], __('{file} does not have a valid file type. Accepted file types are: {extensions}.'));
					}
					else {
						$error_array['file[]'] = __('Could not upload file(s):').'<br>'."\n".str_replace(['{file}', '{extensions}'], [htmlspecialchars($_FILES['file']['name'][$i]), implode(', ', $accepted_extensions)], __('{file} does not have a valid file type. Accepted file types are: {extensions}.'));
					}
			...
			elseif ($_FILES['file']['name'][$i]) {
				// File is not uploaded, but DOES have a name, so something definitely went wrong here. Let's call the whole thing off.
					$errors = true;
					if (isset($error_array['file[]'])) {
						$error_array['file[]'] .= '<br>'."\n".str_replace('{file}', htmlspecialchars($_FILES['file']['name'][$i]), __('{file} could not be uploaded because something went horribly wrong.'));
					}
					else {
						$error_array['file[]'] = __('Could not upload file(s):').'<br>'."\n".str_replace('{file}', htmlspecialchars($_FILES['file']['name'][$i]), __('{file} could not be uploaded because something went horribly wrong (one reason this may happen is if the file size is obscenely large, in which case you should definitely reduce it).'));
					}
		...
		if (!$realimgnum) {
			...
			$errors = true;
			$error_array['file[]'] = __('You haven\'t chosen any files to upload!');
		}
